...
export default function PrintableResult({ result, resultData, version = 'teen' }) {
  ...
  const heroImageSrc = getHeroImageSrc(hero)

  return (
    <div className="printable-result print-only">
      ...
      <section className="printable-result__section printable-result__hero">
        <h3 className="printable-result__section-title">나와 닮은 위인</h3>
        <div className="printable-result__hero-card">
          {heroImageSrc && (
            <img
              src={heroImageSrc}
              alt={hero}
              className="printable-result__hero-image"
              onError={(e) => { e.currentTarget.style.display = 'none' }}
            />
          )}
          <div className="printable-result__hero-text">
            <p className="printable-result__hero-name">{hero}</p>
            <p className="printable-result__hero-desc">{heroDescription}</p>
          </div>
        </div>
      </section>
